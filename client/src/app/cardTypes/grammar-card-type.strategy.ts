...
interface GrammarSentenceIdResponse {
  id: string;
  exists: boolean;
}
...
export class GrammarCardType implements CardTypeStrategy {
    ...
    const grammarSentenceList = await this.multiModelService.call<GrammarSentenceList>(
      ...
      (model: string) =>
        fetchJson<GrammarSentenceList>(
          this.http,
          `/api/source/${sourceId}/page/${pageNumber}/grammar-sentences?x=${x}&y=${y}&width=${width}&height=${height}&model=${model}`
        )
    );

    const sentencesWithIds = await Promise.all(
      grammarSentenceList.sentences.map(async (item) => {
        const sentenceIdResponse = await fetchJson<GrammarSentenceIdResponse>(
          this.http,
          '/api/sentence-id',
          {
  ...
  getItemLabel(item: ExtractedItem & { sentence?: string; gaps?: GapData[] }): string {
    if (!item.sentence) return item.id;
    return this.getSentenceWithGaps(item.sentence, item.gaps);
  }
  ...
  async createCardData(
    ...
    try {
      progressCallback(20, 'Translating to English...');
  ...
  getCardDisplayLabel(card: Card): string {
    return this.getSentenceWithGaps(card.data.sentence, card.data.gaps);
  }
  ...
  private getSentenceWithGaps(sentence?: string, gaps?: GapData[]): string {
    ...
    sortedGaps.forEach((gap) => {
      const placeholder = '_'.repeat(gap.text.length);
