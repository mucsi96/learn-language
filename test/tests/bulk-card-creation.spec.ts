...
test('regions from different pages are combined into single extraction', async ({ page }) => {
  ...
  await page.goto('/sources/cross-page-source/page/1');
...
test('bulk card creation includes word data', async ({ page }) => {
  ...
  await withDbConnection(async (client) => {
    ...
    expect(cardData.examples[0].images[0].model).toBe('Ideogram 4 (Quality)');
...
test('bulk card creation skips image generation for models with zero image count', async ({ page }) => {
  await setupDefaultChatModelSettings();
  await createImageModelSetting({ modelName: 'ideogram-4-quality', imageCount: 2 });
  await createRateLimitSetting({ key: 'image-per-minute', value: 60 });
  await page.goto('/sources');
  await page.getByRole('button', { name: 'Actions for Goethe A1' }).click();
  await page.getByRole('menuitem', { name: 'Pages' }).click();
  ...
  await withDbConnection(async (client) => {
    ...
    expect(cardData.examples[0].images.every((img: { model: string }) => img.model === 'Ideogram 4 (Quality)')).toBe(true);
    expect(cardData.examples[1].images.length).toBe(2);
    expect(cardData.examples[1].images.every((img: { model: string }) => img.model === 'Ideogram 4 (Quality)')).toBe(true);
  });
});
