...
test('dictionary lookup creates draft card visible on cards page', async ({ page, baseURL }) => {
  ...
  await expect(async () => {
    await page.goto('/sources/mein-erstes-buch/cards?filter=draft');
    const grid = page.getByRole('grid');
    const rows = await getGridData(grid);
    expect(rows.length).toBe(1);
    expect(rows[0].Readiness).toBe('DRAFT');
...
test('completes selected draft cards from cards table', async ({ page, baseURL }) => {
  ...
  await expect(async () => {
    await page.goto('/sources/test-buch/cards?filter=draft');
...
test('quick filter flagged shows only flagged cards', async ({ page }) => {
  ...
  await page.goto('/sources/goethe-a1/cards?filter=flagged');
...
test('quick filter suggested known shows only cards meeting criteria', async ({ page }) => {
  ...
  await page.goto('/sources/goethe-a1/cards?filter=suggestedKnown');
