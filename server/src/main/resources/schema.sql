...
-- Migration: Drop old known_words table if it has the old structure
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'learn_language'
        AND table_name = 'known_words'
        AND column_name = 'word'
    ) THEN
        DROP TABLE learn_language.known_words;
    END IF;
END $$;

CREATE TABLE IF NOT EXISTS learn_language.known_words (
    id character varying(255) NOT NULL,
    german character varying(255) NOT NULL,
    hungarian character varying(255) NOT NULL,
