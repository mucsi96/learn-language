...
VALUES ('image-per-minute', 6), ('audio-per-minute', 12),
       ('image-max-concurrent', 2), ('audio-max-concurrent', 3)
...
CREATE TABLE IF NOT EXISTS learn_language.quota_limit_hits (
    id integer NOT NULL GENERATED BY DEFAULT AS IDENTITY,
    ...
    error_message text,
    created_at timestamp(6) NOT NULL,
    CONSTRAINT quota_limit_hits_pkey PRIMARY KEY (id)
);

CREATE INDEX IF NOT EXISTS idx_quota_limit_hits_created_at ON learn_language.quota_limit_hits (created_at);
