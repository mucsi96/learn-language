...
-- Rename review_datetime to review if it exists
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'learn_language' AND table_name = 'review_logs' AND column_name = 'review_datetime') THEN
        ALTER TABLE learn_language.review_logs RENAME COLUMN review_datetime TO review;
    END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS learn_language.study_settings (
    id integer NOT NULL,
    study_mode character varying(50) NOT NULL DEFAULT 'SOLO',
    CONSTRAINT study_settings_pkey PRIMARY KEY (id)
);
