...
CREATE TABLE IF NOT EXISTS learn_language.known_words (
    ...
    german_word character varying(255) NOT NULL,
    hungarian_translation character varying(255) NOT NULL,
    CONSTRAINT known_words_pkey PRIMARY KEY (id),
    CONSTRAINT known_words_word_id_unique UNIQUE (word_id)
);

ALTER TABLE learn_language.known_words ADD COLUMN IF NOT EXISTS word_id character varying(255);
ALTER TABLE learn_language.known_words ADD COLUMN IF NOT EXISTS german_word character varying(255);
ALTER TABLE learn_language.known_words ADD COLUMN IF NOT EXISTS hungarian_translation character varying(255);

DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'learn_language' AND table_name = 'known_words' AND column_name = 'word') THEN
        UPDATE learn_language.known_words
        SET word_id = word, german_word = word, hungarian_translation = ''
        WHERE word_id IS NULL;
    END IF;
END $$;

ALTER TABLE learn_language.known_words DROP CONSTRAINT IF EXISTS known_words_unique;
ALTER TABLE learn_language.known_words DROP COLUMN IF EXISTS word;

DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'known_words_word_id_unique') THEN
        ALTER TABLE learn_language.known_words ADD CONSTRAINT known_words_word_id_unique UNIQUE (word_id);
    END IF;
END $$;
