...
- A native image generates no bytecode at runtime, so Hibernate's
  ...
  group, fails with `JpaSystemException: Generation of HibernateProxy instances
  at runtime is not allowed`. `hibernate-maven-plugin` enhances the entities at
  build time instead, replacing those proxies with field interception, which is
  why the associations can stay lazy - making them eager would change what the
  application queries. The enhancement is not native-specific, so the JVM build
  gets the same classes.
...
- Those libraries call back into Java over JNI, and JNI lookups need their own
  ...
  The same file carries the ICC colour profiles `java.desktop` ships as module
  resources: `ICC_Profile` loads them by name, and without them rendering a
  CMYK PDF page dies inside LCMS with `CMMException: Invalid profile: null`.
